...
func drawSesame(
	...
	// Add the outer edge
	outerEdge, err := canvas.ParseSVGPath("M 0 10.75 A 0.25 0.25 0 0 0 0.25 11 L 7.496 11 A 8.039 ...")
	if err != nil {
	...
	roundedEdge, err := canvas.ParseSVGPath("M 0.2 10.75 A 0.05 0.05 0 0 0 0.25 10.8 L 7.496 ...")
	if err != nil {
		log.Fatalf("Failed to parse SVG path: %s", err)
	}
	...
	// Add text
	builder.StartGroup("Text", map[string]string{})
	for i, line := range lines {
		...
		textPath = textPath.Scale(1, -1).Translate(0, height)
...
// 		log.Fatalf("Failed to parse SVG path: %s", err)
// 	}
// 	outerEdgePathData := outerEdge.ToSVG()
// 	buf.WriteString(`<g inkscape:groupmode="layer" inkscape:label="Outside">`)
// 	buf.WriteString(fmt.Sprintf(`<path d="%s" fill="%s" id="outer-edge" />`, outerEdgePathData, backgroundColor))
// 	buf.WriteString(`</g>`)
...
// 		log.Fatalf("Failed to parse SVG path: %s", err)
// 	}
// 	roundedEdgePathData := roundedEdge.ToSVG()
// 	buf.WriteString(`<g inkscape:groupmode="layer" inkscape:label="Round">`)
// 	buf.WriteString(fmt.Sprintf(`<path d="%s" fill="%s" id="rounded-edge" />`, roundedEdgePathData, foregroundColor))
// 	buf.WriteString(`</g>`)
...
// 		log.Fatalf("Failed to parse SVG path: %s", err)
// 	}
// 	borderOuterPathData := borderOuter.ToSVG()
// 	buf.WriteString(`<g inkscape:groupmode="layer" inkscape:label="Vcarve">`)
// 	buf.WriteString(fmt.Sprintf(`<path d="%s" fill="%s" id="my-custom-id" />`, borderOuterPathData, backgroundColor))
...
// 		log.Fatalf("Failed to parse SVG path: %s", err)
// 	}
// 	borderInnerPathData := borderInner.ToSVG()
// 	buf.WriteString(fmt.Sprintf(`<path d="%s" fill="%s" id="my-custom-id" />`, borderInnerPathData, foregroundColor))
...
// 		if i == 0 {
// 			container = canvas.Rectangle(width-6.0, 3.75)
// 			containerBounds = container.Bounds()
// 			containerX = width/2 - containerBounds.W()/2
// 			containerY = 5.25
// 		} else if i == 1 {
// 			container = canvas.Rectangle(width-2, 2.75)
// 			containerBounds = container.Bounds()
// 			containerX = width/2 - containerBounds.W()/2
// 			containerY = 2.25
// 		} else if i == 2 {
// 			container = canvas.Rectangle(width-2, 1.0)
// 			containerBounds = container.Bounds()
// 			containerX = width/2 - containerBounds.W()/2
// 			containerY = 1.0
// 		}
...
// 		// containerPathData := container.ToSVG()
// 		// buf.WriteString(fmt.Sprintf(`<path d="%s" stroke="%s" stroke-width="0.025" fill="none" id="my-custom-id" />`, containerPathData, "pink"))
...
// 		textPathData := textPath.ToSVG()
// 		buf.WriteString(fmt.Sprintf(`<path d="%s" fill="%s" id="my-custom-id" />`, textPathData, backgroundColor))
// 	}
