	...
	if numLines > 0 {
		...
		case 3:
			containerHeights = []float64{
				availableHeight * 0.21, // First line
				availableHeight * 0.54, // Middle line (taller)
				availableHeight * 0.25, // Last line
			}
		case 2:
			containerHeights = []float64{
				availableHeight * 0.74, // First line (taller)
				availableHeight * 0.26, // Last line
			}
		default:
		...
		for i, line := range lines {
			...
			xOffset := 0.2
			yOffset := 0.075
			x := containerX + containerBounds.W()/2 - textBounds.W()/2 - xOffset*scale
...
func drawDeco(
	...
	if size == "medium" {
		drawMediumDeco(builder, foregroundColor, backgroundColor, strokeOnly)
	} else if size == "large" {
